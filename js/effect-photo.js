...
const EFFECT = {
  // options:[min, max, start, step, effect, unit]
  ...
  chrome: {
    ...
    options: [0, 1, 1, 0.1, 'grayscale', ''],
  },
  sepia: {
    name: 'sepia',
    className: 'effects__preview--sepia',
    options: [0, 1, 1, 0.1, 'sepia', ''],
  },
  marvin: {
    name: 'marvin',
    className: 'effects__preview--marvin',
    options: [0, 100, 100, 1, 'invert', '%'],
  },
  phobos: {
    name: 'phobos',
    className: 'effects__preview--phobos',
    options: [0, 3, 3, 0.1, 'blur', 'px'],
  },
  heat: {
    name: 'heat',
    className: 'effects__preview--heat',
    options: [1, 3, 3, 0.1, 'brightness', ''],
  },
};
...
const onPhotoEffectChange = (evt) => {
  ...
  if (target && target.value === EFFECT.chrome.name) {
    ...
    createSlider(...EFFECT.chrome.options);
  } else {
  ...
  if (target && target.value === EFFECT.sepia.name) {
    ...
    createSlider(...EFFECT.sepia.options);
  } else {
  ...
  if (target && target.value === EFFECT.marvin.name) {
    ...
    createSlider(...EFFECT.marvin.options);
  } else {
  ...
  if (target && target.value === EFFECT.phobos.name) {
    ...
    createSlider(...EFFECT.phobos.options);
  } else {
  ...
  if (target && target.value === EFFECT.heat.name) {
    ...
    createSlider(...EFFECT.heat.options);
  } else {
